...
async def forge_stats(user: str = Depends(get_current_user)):
    """
...
async def forge_logs(
    ...
    try:
        ...
        all_logs = request.app.state.engine.runtime.get_all_logs() if hasattr(request.app.state.engine.runtime, "get_all_logs") else []
...
async def forge_logs_stream(user: str = Depends(get_current_user)):
    """
